...
export class AggregateWeatherData {
    ...
    constructor(dateWeatherList: any[]) {
        dateWeatherList.forEach(dateWeather => {
            ...
            this.weatherCnt[key] = this.weatherCnt[key] !== undefined ? this.weatherCnt[key] + 1 : 1;
            ...
            this.rainList.push(!this.isObjectEmpty(dateWeather.rain) ? dateWeather.rain['3h'] : 0);
            this.snowList.push(!this.isObjectEmpty(dateWeather.snow) ? dateWeather.snow['3h'] : 0);
    ...
    public getWeather(): string {
        ...
        for (var key in this.weatherCnt) {
            if(this.weatherCnt[key] > mostWeatherIdCnt) {
                mostWeatherIdCnt = this.weatherCnt[key];
                mostWeatherId = Number(key);
            }
        }
        var conditionCode = new ConditionCodes();
        const condition:  {id: number; meaning: string; icon: string} = conditionCode.get(mostWeatherId);
    ...
    public getRain(): string {
        return `過去3時間の平均雨量：${this.orgRound(this.average(this.rainList), 100)} mm`;
    }
    ...
    public getSnow(): string {
        return `過去3時間の平均積雪量：${this.orgRound(this.average(this.snowList), 100)}`;
    }
