...
export class AggregateWeatherData {
  ...
  constructor(dateWeatherList: any[]) {
    dateWeatherList.forEach(dateWeather => {
      let key: number = dateWeather.weather[0].id;
      this.weatherCnt[key] =
        this.weatherCnt[key] !== undefined ? this.weatherCnt[key] + 1 : 1;
      ...
      this.rainList.push(
        !this.isObjectEmpty(dateWeather.rain) ? dateWeather.rain['3h'] : 0
      );
      this.snowList.push(
        !this.isObjectEmpty(dateWeather.snow) ? dateWeather.snow['3h'] : 0
      );
    });
  }
